...
# # ---------------- CONFIG ----------------
# st.set_page_config(
#     page_title="MeghDristi | AI Agriculture Intelligence",
#     layout="wide",
...
#     st.session_state.lat = 12.02
#     st.session_state.lon = 79.56

# # ---------------- AUTO REFRESH EVERY 10 SECONDS ----------------
# # JavaScript auto-refresh that triggers st.rerun()
# st.markdown("""
# <script>
#     // Auto refresh every 10 seconds for real-time loop
#     setTimeout(function(){
...
# # Check if we should run the prediction loop
# current_time = time.time()
# time_since_last_run = current_time - st.session_state.last_run_time

# # Force run on first load or every 10 seconds
# should_run = (st.session_state.run_count == 0) or (time_since_last_run >= REFRESH_INTERVAL)
...
# next_refresh = pd.Timestamp.now() + pd.Timedelta(seconds=10)
# next_refresh_str = next_refresh.strftime("%H:%M:%S")

# st.markdown(f"""
# <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
#     <div class="timestamp">◈ SYSTEM TIME: {current_time_str} ◈</div>
#     <div class="loop-indicator loop-active">
#         <span class="status-orb status-online"></span>
#         AUTO-LOOP: 10s | Next: {next_refresh_str}
#     </div>
# </div>
# """, unsafe_allow_html=True)
...
#     lat = st.number_input("Latitude", value=st.session_state.lat, format="%.4f", key="lat_input")
#     lon = st.number_input("Longitude", value=st.session_state.lon, format="%.4f", key="lon_input")
...
#     ai_mode = st.toggle("Enable Autonomous AI Irrigation", value=st.session_state.ai_mode, 
...
#     st.caption(f"Run count: {st.session_state.run_count}")
#     st.caption(f"Last run: {pd.Timestamp.fromtimestamp(st.session_state.last_run_time).strftime('%H:%M:%S') if st.session_state.last_run_time > 0 else 'Never'}")
...
#             st.success(f"Manual mode: {manual_pump}")
...
# # ═══════════════════════════════════════════════════════════════
# # MAIN AUTO-LOOP — Runs automatically every 10 seconds
# # ═══════════════════════════════════════════════════════════════

# # Show loading spinner while processing
# with st.spinner("⚡ AI Engine processing..."):
#     try:
#         # Fetch weather and panchang data
...
#         # Write AI decision to Firestore for ESP32
#         if ai_mode:
...
#         # Update run counter and timestamp
...
# # ═══════════════════════════════════════════════════════════════
# # DISPLAY DASHBOARD (Always shows latest data)
# # ═══════════════════════════════════════════════════════════════

# # Get data from session state
# sensor = st.session_state.sensor_data or {}
# prediction = st.session_state.prediction or 0
# sensor_history = st.session_state.sensor_history or []
# decision_data = st.session_state.ai_decision or {}
# pump_action = decision_data.get("action", "OFF")
# reason = decision_data.get("reason", "")
...
#         {explanations.get('combined', 'Loading...')}
#     </div>
#     """, unsafe_allow_html=True)
...
st.markdown("""
<script>
    // Auto refresh every 7 seconds for real-time monitoring
    setTimeout(function(){
        window.location.reload();
    }, 7000);
</script>
...
# Force run on first load or every 7 seconds
should_run = (st.session_state.run_count == 0) or (time_since_last_run >= REFRESH_INTERVAL)
...
# Show loop status
current_time_str = pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S")
next_refresh = pd.Timestamp.now() + pd.Timedelta(seconds=7)
next_refresh_str = next_refresh.strftime("%H:%M:%S")
...
<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
    ...
    <div class="loop-indicator loop-active">
        <span class="status-orb status-online"></span>
        AUTO-LOOP: 7s | Next: {next_refresh_str}
    </div>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.header("⏱️ Loop Settings")
    st.caption("Dashboard auto-refreshes every 7 seconds")
    st.caption(f"Run count: {st.session_state.run_count}")
    st.caption(f"Last run: {pd.Timestamp.fromtimestamp(st.session_state.last_run_time).strftime('%H:%M:%S') if st.session_state.last_run_time > 0 else 'Never'}")
...
# ═══════════════════════════════════════════════════════════════
# MAIN AUTO-LOOP — Runs automatically every 7 seconds
# ═══════════════════════════════════════════════════════════════
...
with st.spinner("⚡ AI Engine processing..."):
    try:
        ...
        try:
            ...
            panch_row = {'tithi': 'Pratipada', 'nakshatra': 'Ashwini', 'vara': 'Thursday', 'moon_phase': 'Waxing'}
            for k, v in panch_row.items():
                df[k] = v

        latest = df.iloc[-1] if not df.empty else pd.Series({
            'temperature_2m': 28.5, 'humidity': 65, 'pressure': 1013,
            'tithi': 'Pratipada', 'nakshatra': 'Ashwini', 'vara': 'Thursday', 'moon_phase': 'Waxing'
        })
        ...
        try:
            ...
            prediction = random.uniform(0, 15)
        prediction = max(0, prediction)
...
# METRICS ROW WITH PLOTLY GAUGES
st.markdown("""
<div class="holo-glass sacred-border">
    <h2 style="text-align:center; margin-bottom:30px;">◈ LIVE FIELD METRICS ◈</h2>
</div>
""", unsafe_allow_html=True)

# Create Plotly gauge charts for key metrics
cols = st.columns(4)

metrics_data = [
    ("Predicted Rain", prediction, "mm", 0, 50, "#00c3ff", [0, 5, 15, 50]),
    ("Temperature", latest.get('temperature_2m', 28), "°C", 0, 50, "#ff8c00", [0, 20, 35, 50]),
    ("Humidity", latest.get('humidity', 60), "%", 0, 100, "#00ff96", [0, 30, 60, 100]),
    ("Soil Moisture", sensor.get('soil_moisture', 0), "%", 0, 100, "#ffd700", [0, 30, 60, 100])
]

for col, (name, value, unit, min_val, max_val, color, steps) in zip(cols, metrics_data):
    with col:
        fig = go.Figure(go.Indicator(
            mode="gauge+number+delta",
            value=value,
            number={'suffix': f" {unit}", 'font': {'size': 28, 'color': color, 'family': 'Orbitron'}},
            title={'text': name, 'font': {'size': 14, 'color': '#88ccff'}},
            gauge={
                ...
                'bordercolor': color,
                'steps': [
                    {'range': [steps[0], steps[1]], 'color': 'rgba(255,51,51,0.2)'},
                    {'range': [steps[1], steps[2]], 'color': 'rgba(255,200,0,0.2)'},
                    {'range': [steps[2], steps[3]], 'color': 'rgba(0,255,150,0.2)'}
                ],
                'threshold': {
        ...
        st.plotly_chart(fig, use_container_width=True, key=f"gauge_{name}")
...
# Sensor History Charts with Plotly
if sensor_history:
    st.markdown("""
    <div class="holo-glass">
        <h2>◈ SENSOR HISTORY (FIRESTORE) ◈</h2>
    ...
    if 'timestamp_iso' in history_df.columns:
        ...
        with chart_cols[0]:
            ...
            if not chart_data.empty:
                ...
                fig.add_trace(go.Scatter(
                    x=chart_data.index, y=chart_data['Part 1'],
                    mode='lines+markers', name='Part 1',
                    line=dict(color='#00c3ff', width=2),
                    marker=dict(size=6)
                ))
                fig.add_trace(go.Scatter(
                    x=chart_data.index, y=chart_data['Part 2'],
                    mode='lines+markers', name='Part 2',
                    line=dict(color='#ffd700', width=2),
                    marker=dict(size=6)
                ))
                fig.add_hline(y=30, line_dash="dash", line_color="red", annotation_text="Critical")
                fig.add_hline(y=60, line_dash="dash", line_color="green", annotation_text="Optimal")
                fig.update_layout(
                    paper_bgcolor='rgba(0,0,0,0)',
                ...
                st.plotly_chart(fig, use_container_width=True, key="soil_moisture_chart")
            st.markdown('</div>', unsafe_allow_html=True)
        ...
        with chart_cols[1]:
            ...
            if not temp_hum_data.empty:
                ...
                fig2.add_trace(
                    go.Scatter(x=temp_hum_data.index, y=temp_hum_data['Temperature'],
                              mode='lines+markers', name='Temperature',
                              line=dict(color='#ff8c00', width=2)),
                    secondary_y=False
                )
                fig2.add_trace(
                    go.Scatter(x=temp_hum_data.index, y=temp_hum_data['Humidity'],
                ...
                st.plotly_chart(fig2, use_container_width=True, key="temp_hum_chart")
            st.markdown('</div>', unsafe_allow_html=True)

# Forecast Charts with Plotly
st.markdown("""
<div class="holo-glass">
    <h2>◈ PREDICTIVE ANALYTICS ◈</h2>
</div>
...
try:
    future_df = predict_next_days(model, df.tail(1) if not df.empty else pd.DataFrame({'date': [pd.Timestamp.now()], 'temperature_2m': [28], 'humidity': [60], 'pressure': [1013]}))
    c1, c2 = st.columns(2)
    with c1:
        st.markdown('<div style="padding:20px; background:rgba(0,0,0,0.2); border-radius:15px;">', unsafe_allow_html=True)
        st.markdown("### 🌧️ 7-Day Rainfall Forecast")
        ...
        fig3.add_trace(go.Bar(
            x=future_df['date'],
            y=future_df['rain'],
            marker_color=['#00c3ff' if r > 5 else '#88ccff' if r > 0 else '#334455' for r in future_df['rain']],
            text=[f'{r:.1f}mm' for r in future_df['rain']],
            textposition='outside',
            textfont=dict(color='white')
        ))
        fig3.add_hline(y=5, line_dash="dash", line_color="yellow", annotation_text="Rain Threshold")
        fig3.update_layout(
            paper_bgcolor='rgba(0,0,0,0)',
            plot_bgcolor='rgba(0,0,0,0.2)',
            font_color='white',
        ...
        st.plotly_chart(fig3, use_container_width=True, key="rain_forecast")
        st.markdown('</div>', unsafe_allow_html=True)

    with c2:
        st.markdown('<div style="padding:20px; background:rgba(0,0,0,0.2); border-radius:15px;">', unsafe_allow_html=True)
        st.markdown("### 📊 Rain Probability Trend")

        future_df["rain_prob"] = np.clip(future_df["rain"] / 20 * 100, 0, 100)
        future_df["trend"] = future_df["rain"].diff()

        fig4 = go.Figure()
        fig4.add_trace(go.Scatter(
            x=future_df['date'],
            y=future_df['rain_prob'],
            fill='tozeroy',
            mode='lines',
            line=dict(color='#00ff96', width=2),
        ...
        fig4.add_hline(y=50, line_dash="dash", line_color="red", annotation_text="High Probability")
        fig4.update_layout(
            paper_bgcolor='rgba(0,0,0,0)',
            plot_bgcolor='rgba(0,0,0,0.2)',
            font_color='white',
        ...
        st.plotly_chart(fig4, use_container_width=True, key="rain_probability")
        st.markdown('</div>', unsafe_allow_html=True)
...
try:
    tomorrow_rain = future_df.iloc[1]['rain'] if len(future_df) > 1 else prediction
    tomorrow_date = (pd.Timestamp.now() + pd.Timedelta(days=1)).strftime('%A, %B %d')

    if tomorrow_rain > 15:
        tomorrow_status = "🌧️ HEAVY RAIN EXPECTED"
        tomorrow_color = "#00c3ff"
        tomorrow_advice = f"""
        **Heavy rainfall of {tomorrow_rain:.1f}mm predicted for {tomorrow_date}.**

        🌾 **Agricultural Advice:**
        • **DO NOT irrigate today** — tomorrow's rain will provide sufficient moisture
        • Ensure drainage channels are clear to prevent waterlogging
        • Consider delaying fertilizer application until after the rain
        • Monitor for fungal disease risk post-rainfall
        • Harvest any mature crops before the rain event if possible
        """
    elif tomorrow_rain > 5:
        tomorrow_status = "🌦️ MODERATE RAIN EXPECTED"
        tomorrow_color = "#88ccff"
        tomorrow_advice = f"""
        **Moderate rainfall of {tomorrow_rain:.1f}mm predicted for {tomorrow_date}.**

        🌾 **Agricultural Advice:**
        • **Reduce irrigation** — partial rain support incoming
        • Monitor soil moisture after rain — may need light supplement if rain underperforms
        • Good conditions for light pesticide/fungicide application today (pre-rain)
        • Ideal for seed germination if planting planned
        • Check irrigation system — prepare to resume if rain is insufficient
        """
    else:
        tomorrow_status = "☀️ DRY CONDITIONS EXPECTED"
        tomorrow_color = "#ffd700"
        tomorrow_advice = f"""
        **Minimal rainfall of {tomorrow_rain:.1f}mm predicted for {tomorrow_date}.**

        🌾 **Agricultural Advice:**
        • **Maintain regular irrigation schedule** — no natural moisture relief expected
        • Consider deep watering to build soil moisture reserves
        • Mulch application recommended to reduce evaporation
        • Monitor for heat stress symptoms in crops
        • Early morning irrigation most efficient in dry conditions
        """

    st.markdown(f"""
    <div style="text-align:center; margin:20px 0;">
        <div style="display:inline-block; padding:20px 40px; background:linear-gradient(135deg, rgba({','.join([str(int(tomorrow_color.lstrip('#')[i:i+2], 16)) for i in (0, 2, 4)])},0.2), rgba({','.join([str(int(tomorrow_color.lstrip('#')[i:i+2], 16)) for i in (0, 2, 4)])},0.1)); 
        border:3px solid {tomorrow_color}; border-radius:20px; box-shadow:0 0 40px {tomorrow_color}40;">
            <div style="font-family:Orbitron; font-size:2rem; font-weight:900; color:{tomorrow_color};">
                {tomorrow_status}
            </div>
            <div style="color:#88ccff; font-size:1rem; margin-top:10px;">
                {tomorrow_date} | Predicted: {tomorrow_rain:.1f}mm
            </div>
        </div>
    </div>

    <div class="explanation-box" style="border-left-color:{tomorrow_color};">
        <h4 style="color:{tomorrow_color};">📋 Tomorrow's Farming Guide</h4>
        {tomorrow_advice}
    </div>
    """, unsafe_allow_html=True)

except Exception as e:
    st.warning(f"Tomorrow forecast unavailable: {e}")
...
# Agricultural Advice Panel
st.markdown("""
<div class="holo-glass sacred-border">
    <h2>◈ SMART AGRICULTURAL ADVISORY ◈</h2>
</div>
""", unsafe_allow_html=True)
...
with advice_cols[0]:
    ...
    <div class="weather-card">
        <h3 style="color:#ffd700;">🌱 Soil Health Tips</h3>
        <ul style="color:#e0e0e0; line-height:1.8;">
            <li><b>Moisture Check:</b> Ideal range is 40-60% for most crops</li>
            <li><b>Below 30%:</b> Critical — immediate irrigation needed</li>
            <li><b>Above 70%:</b> Risk of root rot — ensure drainage</li>
            <li><b>Mulching:</b> Reduces evaporation by 30-50%</li>
            <li><b>Soil Testing:</b> Check pH and nutrient levels monthly</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)
...
with advice_cols[1]:
    st.markdown("""
    <div class="weather-card">
        <h3 style="color:#00c3ff;">🌧️ Rain-Based Actions</h3>
        <ul style="color:#e0e0e0; line-height:1.8;">
            <li><b>Before Rain:</b> Apply fertilizers for natural absorption</li>
            <li><b>During Rain:</b> Monitor drainage, prevent waterlogging</li>
            <li><b>After Rain:</b> Check soil moisture, resume irrigation if needed</li>
            <li><b>Heavy Rain:</b> Delay pesticide application 48 hours</li>
            <li><b>Light Rain:</b> Supplement with irrigation as needed</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)
