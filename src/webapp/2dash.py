...
st.markdown(f'<div class="timestamp">◈ SYSTEM TIME: {current_time} ◈ AUTO-REFRESH: 8s ◈</div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    # ESP32 Connection with Visual Status
...
def get_sensor_data():
    try:
        ...
        try:
            ...
            # Try parsing HTML if JSON fails
            html_text = res.text
            import re
        ...
        return {
            "soil_moisture": random.uniform(25, 45),  # Simulated for demo
...
<div style="text-align:center; padding:40px; margin-top:50px; border-top:2px solid rgba(0,195,255,0.2);">
    ...
    <p style="color:#00c3ff; font-size:0.9rem; margin-top:20px;">
        Panchang • Machine Learning • IoT Sensors • Predictive Analytics
    </p>
